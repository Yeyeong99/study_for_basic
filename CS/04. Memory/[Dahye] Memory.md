...
웹 개발과 디자인에서는 RGB 값을 16진수(HEX 코드)로 변환하여 사용 
...
웹 디자인, 포토샵 등에서 RGB 색상을 16진수로 표현
...
이것을 **메모리 주소(Memory Address)**라고 하며, 운영체제(OS)와 프로그램이 데이터를 찾고 관리하는 데 사용됨. 
...
컴퓨터의 메모리는 **RAM(Random Access Memory)**에 저장되며, 이곳에 있는 모든 데이터는 **"주소(Address)"**를 가짐
...
📌 `0x1000`, `0x1004`, `0x1008` 등이 **정보(데이터)의 주소**

---

## **2. 예제: 메모리 주소를 활용하는 C 코드**
...
- C, C++ 같은 언어에서는 **"포인터(pointer)"**를 사용하여 메모리 주소를 직접 조작
- **예제:**
...
- 프로그램이 실행될 때 메모리를 **동적으로 할당**하려면 주소가 필요
- **예제 (C에서 `malloc` 사용)**
...
컴퓨터는 **2진수(0과 1)를 사용**하지만, 메모리 주소는 **16진수(HEX)**로 표시됨
...
C/C++에서는 `&`를 **"주소 연산자"**라고 하며, **변수의 메모리 주소를 가져올 때 사용**
...
📌 **즉, `&` 기호는 "변수의 주소를 가져오는" 역할**

---

### **✅ (2) C/C++에서 포인터 연산**

엠퍼샌드(`&`)는 **포인터(pointer)와 함께 사용**될 때, 변수의 주소를 포인터에 저장
...
Python과 JavaScript에서는 `&`가 **논리 AND 연산**(True/False 계산)에 사용
...
HTML에서는 엠퍼샌드(`&`)를 특수 문자 표현에 사용

예를 들어, `<`(작다)는 HTML에서 그대로 쓰면 코드로 인식되므로 **`&lt;`**로 변환
...
- `typedef char *string;`을 선언하면, `string`이 `char *`의 **별칭**이 됨  
